...
pub mod vault {
    ...
    /// * `expires_at` - Unix timestamp when the deposit expires (0 = never expires)
    ///
    /// # Returns
    ...
    pub fn create_private_deposit(
        ...
        let rent = Rent::get()?;
        let rent_required = rent.minimum_balance(90); // 90 bytes for PrivateDeposit account
        require!(
    ...
    pub fn claim_deposit(
        ...
        // Check if deposit has expired (expires_at = 0 means never expires)
        if deposit.expires_at > 0 {
            let clock = Clock::get()?;
            require!(
    ...
    pub fn refund_expired(
        ...
        // Check that deposit has expired (expires_at must be > 0 and in the past)
        require!(deposit.expires_at > 0, PrivyLinkError::DepositNeverExpires);

        let clock = Clock::get()?;
        require!(
            clock.unix_timestamp >= deposit.expires_at,
            PrivyLinkError::DepositNotExpired
        );

        // Transfer SOL from deposit account back to depositor
        let bump = deposit.bump;
        let depositor_key = deposit.depositor;
        let seeds: &[&[&[u8]]] = &[&[
            b"deposit",
        ...
        // Mark deposit as claimed to prevent double-refund
        let deposit = &mut ctx.accounts.deposit;
        deposit.claimed = true;
...
pub struct PrivateDeposit {
    ...
    /// Unix timestamp when the deposit expires (0 = never expires)
...
// Total: 8 (discriminator) + 32 + 32 + 8 + 8 + 1 + 1 = 90 bytes

/// Context for creating a private deposit.
#[derive(Accounts)]
#[instruction(deposit_id: u64, amount: u64, claim_hash: [u8; 32], expires_at: i64)]
pub struct CreatePrivateDeposit<'info> {
    ...
    #[account(
        init,
        payer = depositor,
        space = 8 + 32 + 32 + 8 + 8 + 1 + 1, // 90 bytes total
        seeds = [
            b"deposit",
...
pub struct RefundExpired<'info> {
    ...
    #[account(
        mut,
        constraint = depositor.key() == deposit.depositor @ PrivyLinkError::NotDepositor
    )]
    pub depositor: Signer<'info>,
...
pub enum PrivyLinkError {
    ...
    DepositExpired,
    /// The deposit has not expired yet (cannot refund)
    #[msg("Deposit has not expired yet")]
    DepositNotExpired,
    /// The deposit never expires (cannot refund)
    #[msg("Deposit never expires")]
    DepositNeverExpires,
    /// Only the original depositor can request a refund
    #[msg("Only depositor can refund")]
    NotDepositor,
}
